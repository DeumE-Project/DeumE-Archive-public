...
<head>
    ...
    <title>packages loadPackages</title>
    ...
    $('#save-btn').on('click', function (e) {
        ...
        if (fileInput.files[0]!=null){
            ...
            $.ajax({
                ...
                success: function (result) {
                    console.log('packages main file s3 upload success: ')
                    console.log(result)
                },
                error: function (err) {
                    console.log('packages main file s3 upload error: ')
                    console.log(err)
                }
            })
        ...
        packagesSaveRequestObj.packagesMainFileDTO = mainUpload;

        console.log(packagesSaveRequestObj);

        $.ajax({
            type: 'post',
            url: '/packages/loadPackages',
            data: JSON.stringify(packagesSaveRequestObj),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            async: false,
            success: function (result) {
                console.log('꾸러미 정보 save success!')
                alert('꾸러미가 등록되었습니다')
                console.log(result)
            },
            error: function (err) {
                console.log('꾸러미 정보 save error!')
                alert('꾸러미 등록하지 못했습니다')
                console.log(err)
            }
        })
        location.href = '/packages/listPackages';
