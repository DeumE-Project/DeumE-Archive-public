        ...
        <div class="inner">
            <div class="table-cnt">
                <div class="table-type02">
                    <div class="table-top">
                        <span class="table-tit">꾸러미 등록</span>
                        <div class="btn-wrap">
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="radio" id="display1" name="display" value="1"
                                       <c:if test="${packagesInfo.display eq 1}">checked</c:if>>
                                <label for="display1">노출</label>
                                <input type="radio" id="display2" name="display" value="0"
                                       <c:if test="${packagesInfo.display eq 0}">checked</c:if>>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" name="name" id="name" value="${packagesInfo.name}">
                            </td>
                        </tr>
                        <tr>
                            <th colspan="2">
                        ...
                        <tr>
                            <th colspan="2">설명</th>
                            <td>
                                <input type="text" name="explanation" id="explanation" value="${packagesInfo.explanation}">
                            </td>
                        </tr>
                        <tr>
                            <th colspan="2">출처</th>
                            <td>
                                <input type="text" name="source" id="source" value="${packagesInfo.source}">
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <select name="maincate" id="maincate">
                                    ...
                                    <option id="maincate1" value="1"
                                            <c:if test="${packagesInfo.subcateIdx le 7}">selected</c:if>>
                                        명절
                                    </option>
                                    <option id="maincate2" value="2"
                                            <c:if test="${packagesInfo.subcateIdx gt 7}">selected</c:if>>
                                        민속놀이
                                    </option>
                                </select>
                ...
                <div class="table-type02">
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="file" id="mainUpload" accept=".zip" value="${packagesInfo.original}"/>
                            </td>
                        </tr>
                        </tbody>
                    </table>
...
<script>
    ...
    const subCateLoad1 = '\
        <option value="">- 서브카테고리 선택 -</option>\
        <option id="subcate1" value="1" <c:if test="${packagesInfo.subcateIdx eq 1}">selected</c:if>>신정</option>\
        <option id="subcate2" value="2" <c:if test="${packagesInfo.subcateIdx eq 2}">selected</c:if>>설날</option>\
        <option id="subcate3" value="3" <c:if test="${packagesInfo.subcateIdx eq 3}">selected</c:if>>정월대보름</option>\
        <option id="subcate4" value="4" <c:if test="${packagesInfo.subcateIdx eq 4}">selected</c:if>>단오</option>\
        <option id="subcate5" value="5" <c:if test="${packagesInfo.subcateIdx eq 5}">selected</c:if>>칠석</option>\
        <option id="subcate6" value="6" <c:if test="${packagesInfo.subcateIdx eq 6}">selected</c:if>>추석</option>\
        <option id="subcate7" value="7" <c:if test="${packagesInfo.subcateIdx eq 7}">selected</c:if>>동지</option>\
        ';
    ...
    const subCateLoad2 = '\
        <option value="">- 서브카테고리 선택 -</option>\
        <option id="subcate8" value="8" <c:if test="${packagesInfo.subcateIdx eq 8}">selected</c:if>>강강술래</option>\
        <option id="subcate9" value="9" <c:if test="${packagesInfo.subcateIdx eq 9}">selected</c:if>>널뛰기</option>\
        <option id="subcate10" value="10" <c:if test="${packagesInfo.subcateIdx eq 10}">selected</c:if>>연날리기</option>\
        <option id="subcate11" value="11" <c:if test="${packagesInfo.subcateIdx eq 11}">selected</c:if>>씨름</option>\
        <option id="subcate12" value="12" <c:if test="${packagesInfo.subcateIdx eq 12}">selected</c:if>>쥐불놀이</option>\
        <option id="subcate13" value="13" <c:if test="${packagesInfo.subcateIdx eq 13}">selected</c:if>>투호</option>\
        <option id="subcate14" value="14" <c:if test="${packagesInfo.subcateIdx eq 14}">selected</c:if>>팽이치기</option>\
        ';
    ...
    // 기존 썸네일 이미지들 받아오기
    const packagesIdx = ${packagesInfo.idx};
    console.log(packagesIdx)
    $.ajax({
        type: 'get',
        url: '/packages/update/get-info/' + packagesIdx,
        dataType: 'json',
        ...
        error: function(err) {
            console.log(packagesIdx + " -> 정보 갖고오기 실패")
            console.log(packagesIdx)
        }
    })

    // 기존의 꾸러미 파일을 이름만 가져오기
    const fileInput = document.querySelector('input[type="file"]');

    const myFile = new File([''], '${packagesInfo.original}', {
        type: 'text/plain'
    });
    ...
    function fillUpThumbnails(packagesThumbnailList) {
        const thumbnailList = packagesThumbnailList
...
<script>
    ...
    function handlePackagesMainFile() {
        const mainFileInfo = {}
        if (fileInput.files[0].name == '${packagesInfo.original}') {
            console.log('파일이 변경되지 않음');
            mainFileInfo.original = fileInput.files[0].name;
            mainFileInfo.saved = '${packagesInfo.saved}';
        } else {
            $.ajax({
                type: 'post',
                url: '/presigned/upload-url',
                ...
                async: false,
                success: function(result) {
                    console.log('packages main file upload presigned url generation success')
                    mainFileInfo.original = fileInput.files[0].name
            ...
            $.ajax({
                ...
                async: false,
                success: function (result) {
                    console.log('packages main update file s3 upload success: ')
                    console.log(result)
                },
                error: function (err) {
                    console.log('packages main update file s3 upload error: ')
                    console.log(err)
                }
            })
        }
    ...
    $('#save-btn').on('click', function (e) {
        ...
        const mainFileInfo = handlePackagesMainFile()
        ...
        const packagesSaveRequestObj = {
            idx: '${packagesInfo.idx}',
        ...
        console.log(packagesSaveRequestObj)

        packagesSaveRequestObj.packagesThumbnailList = thumbnailFileInfoList;
        packagesSaveRequestObj.packagesMainFileDTO = mainFileInfo;
        console.log(packagesSaveRequestObj);

        $.ajax({
            type: 'post',
            url: '/packages/update',
            data: JSON.stringify(packagesSaveRequestObj),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            async: false,
            success: function (result) {
                console.log('꾸러미 정보 update success!')
                alert('꾸러미가 수정되었습니다')
                console.log(result)
            },
            error: function (err) {
                console.log('꾸러미 정보 update error!')
                alert('꾸러미를 수정하지 못했습니다')
                console.log(err)
            }
        })
        location.href = '/packages/detailPackages/' + packagesSaveRequestObj.idx;
    })
</script>
