...
public class ContentsService {
    ...
    public ContentsDTO insertContentsWithThumbnailAndMainImage(ContentsSaveRequestDTO requestDTO) {
    ...
    public List<ContentsThumbnailDTO> selectContentsThumbnailForList() {
        List<ContentsThumbnailDTO> thumbnailList = contentsRepository.selectContentsThumbnailForList();
    ...
    public List<ContentsThumbnailDTO> selectContentsThumbnailForListInMain() {
        List<ContentsThumbnailDTO> thumbnailList = contentsRepository.selectContentsThumbnailForListInMain();
        thumbnailList.stream().forEach((thumbnail) -> {
            PresignedUrlViewRequestDTO requestDTO = new PresignedUrlViewRequestDTO();
            requestDTO.setPath(thumbnail.getSavedPath());
            requestDTO.setFileNameWithUUID(thumbnail.getSaved());
    ...
    public Map<String, Object> downloadMultiMainImage(ContentsMultiMainImageDownloadRequestDTO requestDTO) throws IOException {
        final String filePath = "contents/";
        List<String> fileNameList = contentsRepository.selectContentsFileNameList(requestDTO.getContentsIdxList());
    ...
    public List<ContentsDTO> selectContentsForManageList() {
        List<ContentsDTO> contentsList = contentsRepository.selectContentsForManage();
        contentsList.stream()
                .forEach((contents) -> {
                    contents.getContentsThumbnailList().stream()
                        .forEach((thumbnail) -> {
                            PresignedUrlViewRequestDTO requestDTO = new PresignedUrlViewRequestDTO();
                            requestDTO.setPath(thumbnail.getSavedPath());
                            requestDTO.setFileNameWithUUID(thumbnail.getSaved());
