...
import kr.co.chunjae.contents.dto.request.ContentsDownloadRequestDTO;
import kr.co.chunjae.contents.dto.request.ContentsMultiMainImageDownloadRequestDTO;
...
public class ContentsController {
    ...
    public ResponseEntity<?> contentsSave(@Validated @RequestBody ContentsSaveRequestDTO requestDTO) {
        ...
        ContentsDTO savedContentsDTO = contentsService.insertContentsWithThumbnailAndMainImage(requestDTO);

        Map<String, Object> resultMap = new HashMap<>();
        resultMap.put("message", "콘텐츠가 등록되었습니다");
        resultMap.put("idx", savedContentsDTO.getIdx());

        log.info("savedContentsDTO = {}", savedContentsDTO);
    ...
    public String contentsList(Model model) {
        model.addAttribute("thumbnailList", contentsService.selectContentsThumbnailForList());
        return "contents/list";
    }

    @PostMapping(value = "/download-from-list")
    public ResponseEntity<?> downloadMainImageFromList(@RequestBody ContentsDownloadRequestDTO requestDTO) throws UnsupportedEncodingException {
        ContentsDTO contentsDTO = contentsService.selectContents(requestDTO.getContentsIdx());
        BasicS3DownloadRequestDTO basicS3DownloadRequestDTO = BasicS3DownloadRequestDTO.builder()
                .path(contentsDTO.getUploadPath())
                .fileNameWithUUID(contentsDTO.getSaved())
                .build();
        ...
        return ResponseEntity
                ...
                .header("Content-Disposition",
                        "attachment;filename=" +
                                new String(contentsDTO.getOriginal().getBytes("utf-8"), "ISO-8859-1"))
                .body(byteArrayResource);
    }

    @PostMapping(value = "/multi-download")
    public ResponseEntity<?> downloadMultiMainImagesFromListPage(
            @Validated @RequestBody ContentsMultiMainImageDownloadRequestDTO requestDTO
    ) throws UnsupportedEncodingException {
        Map<String, Object> resultMap = null;
        try {
            resultMap = contentsService.downloadMultiMainImage(requestDTO);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        ByteArrayResource byteArrayResource = (ByteArrayResource) resultMap.get("zipFile");
    ...
    public String manageContents(Model model) {
        List<ContentsDTO> contentsList = contentsService.selectContentsForManageList();
